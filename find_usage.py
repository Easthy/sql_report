...
def get_file_content(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as sql_file:
            ...
            sql_content = ''.join(line for line in content.split('\n') 
...
def find_column_usage(sql_content, target_schema, target_table, target_columns):
    ...
    try:
        ...
        sql_keywords = {
            ...
            'ORDER', 'BY', 'ON', 'AS', 'WITH', 'AND', 'OR', 'NULL', 'HAVING', 'DISTINCT', 
            'LIMIT', 'OFFSET', 'UNION', 'INTERSECT', 'INNER JOIN', 'LEFT JOIN', 'FULL JOIN',
            'FULL OUTER JOIN', 'UNION ALL', 'RIGHT JOIN', 'OUTER JOIN', 'GROUP BY', 'ORDER BY'
        }
        
        # Filtering tables_aliases, excluding keywords
        tables = {k: v for k, v in parser.tables_aliases.items() if k.upper() not in sql_keywords}
        ...
        for table in parser.tables:
            ...
            if table not in tables.values() and table not in tables and table.upper() not in sql_keywords:
                ...
                for sub_ref in column_ref:
                    if isinstance(sub_ref, str):
                        ...
                        if len(parts) >= 2:
                            ...
                            if (f"{target_schema}.{target_table}" in full_table or 
                                target_table == full_table or 
                                full_table.endswith(f".{target_table}")) and column in target_columns:
        ...
        if parser.with_queries:
            ...
            for cte_name, cte_query in parser.with_queries.items():
                ...
                cte_used_columns = find_column_usage(cte_query, target_schema, target_table, target_columns)
...
def analyze_sql_files(folder_path, target_schema, target_table, target_columns):
    ...
    for sql_file_path in sql_file_paths:
        ...
        if sql_content:
            ...
            used_columns = find_column_usage(sql_content, target_schema, target_table, target_columns)
            if used_columns:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--folder_path', type=str, default=folder_path, help='Folder to search SQL code')
    parser.add_argument('--target_schema', type=str, default=target_schema, help='DWH Schema of the target table')
    parser.add_argument('--target_table', type=str, default=target_table, help='The name of the DWH table whose usage we will be searching for')
    parser.add_argument('--target_columns', type=str, default=target_columns, help='The list of columns in the table, the usage of which we will be searching for')

    args = parser.parse_args()

    analyze_sql_files(args.folder_path, args.target_schema, args.target_table, json.loads(args.target_columns))
