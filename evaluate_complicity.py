...
logging.basicConfig(level=logging.INFO, handlers=[RichHandler(rich_tracebacks=True, show_time=False)])
...
def get_table_columns(table_name, db_report_schema):
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(query, (db_report_schema, table_name,))
...
def get_file_content(file_path):
    ...
    with open(file_path, 'r') as sql_file:
        ...
        view_name = re.search(pattern, first_line).group(1) if re.search(pattern, first_line) else None
        sql_content = ''.join(line for line in sql_file if 'WITH NO SCHEMA BINDING' not in line and 'CREATE OR REPLACE VIEW' not in line)
...
def count_sql_elements(sql_content):
    ...
    elements = {
        ...
        "regexp_count": sum(lower_sql.count(func) for func in ['regexp_substr', 'regexp_replace', 'regexp_instr', 'regexp_count'])
    }
    return elements

def count_sql_operators(sql_content):
    operators = ['json_extract_path', 'nvl', 'coalesce', 'group by', 'order by', 'having', 'distinct', 'listagg', 'split_part', 'substring', 'over', 'date_trunc', 'date_part', 'json_parse', 'json_serialize']
...
def analyze_sql_files(file_path, db_search, db_report_schema):
    ...
    for sql_file_path in sql_file_paths:
        ...
        tables_used = [table for table in sql_parsed_lib_sql_metadata.tables if table.lower() not in ('current_date', 'date_trunc', 'current_timestamp', 'case', 'lower', 'nvl', 'count', 'sum', 'position',)]
        ...
        data = {
            ...
            'score': (len(tables_used) * 0.2) + (sql_operators * 0.1) + (sql_elements.get('join_count') * 0.3) + (len(subqueries_used) * 0.5) + (len(cte_used) * 0.5) + (sql_elements.get('case_count') * 0.2) + (sql_elements.get('union_count') * 0.4) + (sql_elements.get('cross_join_count') * 0.7) + (sql_elements.get('regexp_count') * 0.6),
...
if __name__ == "__main__":
    ...
    parser.add_argument('--file_path', type=str, default='sql_code', help='Folder to search SQL code')
    parser.add_argument('--db_config', type=str, default='db_config.yml', help='DB connection config file')
    parser.add_argument('--db_search', type=bool, default=False, help='Search and analyze view at your DWH')
    parser.add_argument('--db_report_schema', type=str, default='public', help='DWH schema of the report view')
